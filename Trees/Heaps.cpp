...
int main(){
    ...
    int n,t; cin>>n;
    vector<int> a;
    for(int i=0; i<n; ++i) cin>>t, h.insert(t), a.push_back(t);
    vector<int> res = h.heapArray;
    cout<<"HeapArray O(nlogn) -> "; for(int i:res) cout<<i<<' '; cout<<'\n';
    cout<<"Deletion order (sorted) -> "; for(int i=0; i<n; ++i) cout<<h.del()<<' '; cout<<'\n';
    h.heapify(a);
    cout<<"heapified Array (O(n))-> "; for(int i:a) cout<<i<<' ';
    return 0;
}
